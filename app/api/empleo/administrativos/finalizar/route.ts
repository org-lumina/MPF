...
import { registrarEmpleoAdminCompletado } from "@/lib/progreso";
import type { ResultadoExamenUI } from "@/lib/tipos";

// POST /api/empleo/administrativos/finalizar — corrige (server) + registra + devuelve.
export async function POST(req: Request) {
  ...
  const examen = cargarEmpleoAdmin().find((x) => x.examen_id === Number(body.examen_id));
  ...
  try { await registrarEmpleoAdminCompletado(email, examen.examen_id); } catch {}
